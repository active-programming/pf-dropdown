...
var pfDropdown = function () {
    function pfDropdown(element) {
        ...
        // load remote data
        if (_jquery2.default.trim(this.settings.ajax.url) !== '' && this.settings.ajax.loadOnInit === true) {
            this._loadRemoteItems();
        }
    }

    _createClass(pfDropdown, [{
        key: '_loadOriginalOptions',
        value: function _loadOriginalOptions() {
            var _this = this;

            var $groups = this.$original.find('optgroup');
            var itemsLoopFn = function itemsLoopFn($options, groupId) {
                $options.each(function (_, o) {
                    var $o = (0, _jquery2.default)(o),
                        dataset = $o.data('set'),
            ...
            if ($groups.length > 0) {
                $groups.each(function (_, g) {
                    var groupId = _this.groups.length + 1;
                    _this.groups.push({ id: groupId, label: (0, _jquery2.default)(g).attr('label') });
                    itemsLoopFn((0, _jquery2.default)(g).find('option'), groupId);
                });
            } else {
        ...
        value: function _getSelectedItem() {
            ...
            var currentValue = this.$original.find('option:selected').attr('value');
        ...
        key: '_implementOriginalStyles',
        value: function _implementOriginalStyles($original, $container) {
            var needed = ['background', 'backgroundColor', 'border', 'position', 'top', 'left', 'right', 'bottom', 'color', 'cursor', 'font', 'height', 'lineHeight', 'margin', 'maxHeight', 'maxWidth', 'outline', /* 'padding', */
            'width', 'wordSpacing', 'wordWrap', 'zoom'];
            var originalStyles = typeof document.defaultView !== 'undefined' ? document.defaultView.getComputedStyle($original[0], null) : {};
            for (var key in originalStyles) {
                if (needed.includes(key)) {
                    var value = originalStyles[key];
                    ...
                    if (key === 'border') {
                        if (value !== 'none') {
                            $container.find('.pf-input-frame').css(key, value).css('box-sizing', 'border-box');
                            $container.find('.pf-dropdown-frame').css(key, value).css('border-top', 'none').css('box-sizing', 'border-box');
                        }
                    } else {
        ...
        value: function _renderContainer($original, settings) {
            $original.css('display', 'none');
            var $container = (0, _jquery2.default)('<div>').addClass(settings.containerClass).append((0, _jquery2.default)(this.containerTmpl));
        ...
        value: function _renderItems(items) {
            var groupId = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : -1;
                ...
                try {
                    for (var _iterator8 = items[Symbol.iterator](), _step8; !(_iteratorNormalCompletion8 = (_step8 = _iterator8.next()).done); _iteratorNormalCompletion8 = true) {
                        ...
                        if (groupId < 0 || groupId >= 0 && item.group == groupId) {
                            var $item = this._renderItem(item);
                            if ($item instanceof _jquery2.default) {
        ...
        value: function _renderItem(item) {
            ...
            if ([_typeof(item.id), _typeof(item.value), _typeof(item.title)].includes('undfined')) return false;
            var $itemOrig = (0, _jquery2.default)(this.itemTmpl).attr('data-item_value', item.value).html(item.title),
                $item = this._executeCallback('renderItem', $itemOrig.clone(), item, this.$original, this.$container, this.settings);
            if (!($item instanceof _jquery2.default) || !$item.hasClass('pf-dropdown-item') || !$item.data('item_value')) {
                $item = $itemOrig;
            }
        ...
        key: '_renderGroup',
        value: function _renderGroup(group, $items) {
            var $groupOrig = (0, _jquery2.default)(this.groupTmpl).attr('data-group_id', group.id);
            $groupOrig.find('.pf-group-item').html(group.label);
            if ($items instanceof _jquery2.default) {
                $groupOrig.find('.pf-dropdown-group-items').html($items);
            }
            var $group = this._executeCallback('renderGroup', $groupOrig.clone(true), group, $items, this.$original, this.$container, this.settings);
            if (!($group instanceof _jquery2.default)) {
                $group = $groupOrig;
            }
            return $group;
        }
        ...
        value: function _toggleDropdown() {
            (0, _jquery2.default)('body').trigger('pf-dropdown-click');
            var $dropdown = this.$container.find('.pf-dropdown-frame');
        ...
        key: '_selectItem',
        value: function _selectItem($item, data) {
            var $input = this.$container.find('.pf-input'),
...
(0, _plugin3.default)('pfDropdown', pfDropdown);
